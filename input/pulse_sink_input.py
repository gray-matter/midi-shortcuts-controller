...
class PulseSinkInput:
    ...
    def __init__(self, app_name_pattern: Optional[re.Pattern], media_name_pattern: Optional[re.Pattern], pulse: PulseAsync):
    ...
    def refresh_sinks(self, input_list: List[PulseSinkInputInfo]):
        ...
        logging.debug(f'Found {len(self._sinks)} sink(s) for application {self._app_name_pattern}')
